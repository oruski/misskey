      ...
      <FormSuspense :p="init">
        <div v-if="tab === 'overview'" class="_gaps_m">
          ...
          <div style="display: flex; flex-direction: column; gap: 1em">
            ...
            <MkKeyValue oneline>
              ...
              <template #value>
                <span class="_monospace"><MkTime :time="user.createdAt" :mode="'detail'" /></span>
              </template>
            </MkKeyValue>
            <MkKeyValue v-if="info" oneline>
              <template #key>{{ i18n.ts.lastActiveDate }}</template>
              <template #value>
                <span class="_monospace"><MkTime :time="info.lastActiveDate" :mode="'detail'" /></span>
              </template>
            </MkKeyValue>
            <MkKeyValue v-if="info" oneline>
              <template #key>{{ i18n.ts.email }}</template>
              <template #value>
                <span class="_monospace">{{ info.email }}</span>
              </template>
            </MkKeyValue>
          </div>
            ...
            <div class="_gaps_m">
              <div style="display: flex; flex-direction: column; gap: 1em">
                <MkKeyValue v-if="user.host" oneline>
                  ...
                  <template #value>
                    <MkA :to="`/instance-info/${user.host}`" class="_link">
                      {{ user.host }} <i class="ti ti-chevron-right"></i>
                    </MkA>
                  </template>
                </MkKeyValue>
                ...
                <MkKeyValue oneline>
                  ...
                  <template #value>
                    <MkTime v-if="user.lastFetchedAt" mode="detail" :time="user.lastFetchedAt" /><span v-else>N/A</span>
                  </template>
                </MkKeyValue>
                <MkKeyValue v-if="ap" oneline>
                  <template #key>Type</template>
                  <template #value>
                    <span class="_monospace">{{ ap.type }}</span>
                  </template>
                </MkKeyValue>
              </div>

              <MkButton v-if="user.host != null" @click="updateRemoteUser">
                <i class="ti ti-refresh"></i> {{ i18n.ts.updateRemoteUser }}
              </MkButton>
        ...
        <div v-else-if="tab === 'moderation'" class="_gaps_m">
          ...
          <MkFolder>
            ...
            <div class="_gaps">
              <MkButton v-if="iAmModerator" primary rounded @click="assignRole">
                <i class="ti ti-plus"></i> {{ i18n.ts.assign }}
              </MkButton>
          ...
          <MkTextarea v-model="suspendedReason" manual-save>
            <template #label>Suspended reason</template>
          </MkTextarea>
        </div>
        ...
        <div v-else-if="tab === 'raw'" class="_gaps_m">
          <MkObjectView v-if="info && $i.isAdmin" tall :value="info"> </MkObjectView>
    ...
    return () =>
      Promise.all([
        ...
        watch($$(suspendedReason), async () => {
          await os.api('admin/update-user-suspended-reason', { userId: user.id, text: suspendedReason });
          await refreshUser();
        });
      });
  } else {
