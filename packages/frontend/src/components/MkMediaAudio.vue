    ...
    <button v-if="hide" :class="$style.hidden" @click="hide = false">
      <div :class="$style.hiddenTextWrapper">
        <b v-if="audio.isSensitive" style="display: block"
          ...
          }}{{ ` (${i18n.ts.audio}${audio.size ? ' ' + bytes(audio.size) : ''})` }}</b
        >
        <b v-else style="display: block"
          ><i class="ti ti-music"></i> {{ audio.size ? bytes(audio.size) : i18n.ts.audio }}</b
        >
        <span style="display: block">{{ i18n.ts.clickToShow }}</span>
      </div>
    </button>
    <div v-else :class="$style.audioControls">
      <audio ref="audioEl" preload="metadata" :class="$style.audio">
      ...
      <MkMediaRange v-model="rangePercent" :class="$style.seekbarRoot" :buffer="bufferedDataRatio" />
    </div>
  </div>
</template>
...
const hide = ref(
  defaultStore.state.nsfw === 'force' || (props.audio.isSensitive && defaultStore.state.nsfw !== 'ignore'),
);
...
onDeactivated(() => {
  ...
  durationMs.value = 0;
  bufferedEnd.value = 0;
  hide.value = defaultStore.state.nsfw === 'force' || (props.audio.isSensitive && defaultStore.state.nsfw !== 'ignore');
  stopAudioElWatch();
  onceInit = false;
});
</script>
