...
export const apiWithDialog = ((endpoint: string, data: Record<string, any> = {}, token?: string | null | undefined) => {
...
export async function selectDriveFile(multiple: boolean) {
  return new Promise((resolve, reject) => {
    popup(
      defineAsyncComponent(() => import('@/components/MkDriveSelectDialog.vue')),
      {
        type: 'file',
        multiple,
      },
...
type AwaitType<T> = T extends Promise<infer U> ? U : T extends (...args: any[]) => Promise<infer V> ? V : T;
...
/**
 * メッセージを送信するボタンをクリックする
 */
export function clickSendMessageButton() {
  const button = document.querySelector('#message-post__button') as HTMLButtonElement;
  button.click();
}
